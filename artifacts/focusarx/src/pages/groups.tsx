...
function GroupCard({ group, onJoin, isMember }: { group: any; onJoin: (id: string) => void; isMember?: boolean }) {
  ...
  return (
    <div className="rounded-2xl border border-[#1e2130] bg-[#111318] p-4 hover:border-[#7C3AED]/40 transition-all">
      <div className="flex items-start gap-3">
        <span className="text-3xl">{group.avatarEmoji || "🎯"}</span>
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            <p className="text-sm font-bold text-[#e8eaf0] truncate">{group.name}</p>
            {group.isPublic ? <Globe size={11} className="text-[#4a4f62] shrink-0" /> : <Lock size={11} className="text-[#4a4f62] shrink-0" />}
          </div>
          {group.description && <p className="text-xs text-[#5a5f72] mt-0.5 line-clamp-2">{group.description}</p>}
          <div className="flex items-center gap-3 mt-2">
            <span className="text-xs text-[#4a4f62]"><Users size={10} className="inline mr-1" />{group.memberCount ?? 0}/{group.maxMembers}</span>
            <span className="text-xs text-[#4a4f62]">⚡ Level {level}</span>
            <span className="text-xs text-[#4a4f62]">{(group.groupXp ?? 0).toLocaleString()} XP</span>
          </div>
          {(group.tags ?? []).length > 0 && (
            <div className="flex flex-wrap gap-1 mt-2">
              {group.tags.slice(0, 3).map((t: string) => <span key={t} className="rounded-full border border-[#1e2130] px-2 py-0.5 text-[9px] text-[#4a4f62]">{t}</span>)}
            </div>
          )}
        </div>
        {!isMember && (
...
function CreateGroupModal({ onClose, onCreate }: { onClose: () => void; onCreate: (data: any) => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4">
      <div className="w-full max-w-md rounded-2xl border border-[#1e2130] bg-[#111318] p-6">
        <div className="flex items-center justify-between mb-5">
          <h2 className="text-lg font-bold text-[#e8eaf0]">Create Study Group</h2>
          <button onClick={onClose} className="text-[#4a4f62] hover:text-[#e8eaf0]"><X size={18} /></button>
        </div>
        <div className="space-y-4">
          <div>
            <label className="text-xs font-semibold uppercase tracking-widest text-[#4a4f62] block mb-1.5">Avatar</label>
            <div className="flex gap-2 flex-wrap">{emojis.map(e => <button key={e} onClick={() => setForm(f => ({ ...f, avatarEmoji: e }))} className={`text-2xl rounded-lg p-1.5 transition-all ${form.avatarEmoji === e ? "bg-[#7C3AED]/30 ring-1 ring-[#7C3AED]" : "hover:bg-[#1e2130]"}`}>{e}</button>)}</div>
          </div>
          <div>
            <label className="text-xs font-semibold uppercase tracking-widest text-[#4a4f62] block mb-1.5">Name *</label>
            <input value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} placeholder="Study group name…" className="w-full rounded-xl border border-[#1e2130] bg-[#0a0c12] px-3 py-2 text-sm text-[#e8eaf0] outline-none focus:border-[#7C3AED]" />
          </div>
          <div>
            <label className="text-xs font-semibold uppercase tracking-widest text-[#4a4f62] block mb-1.5">Description</label>
            <textarea value={form.description} onChange={e => setForm(f => ({ ...f, description: e.target.value }))} placeholder="What's this group about?" rows={2} className="w-full rounded-xl border border-[#1e2130] bg-[#0a0c12] px-3 py-2 text-sm text-[#e8eaf0] outline-none focus:border-[#7C3AED] resize-none" />
          </div>
          <div className="flex gap-4">
            <label className="flex items-center gap-2 cursor-pointer">
              ...
              <span className="text-sm text-[#e8eaf0]">Public</span>
            </label>
            <label className="flex items-center gap-2 cursor-pointer">
              ...
              <span className="text-sm text-[#e8eaf0]">Private (invite only)</span>
            </label>
          </div>
          <div className="flex gap-2">
            <button onClick={onClose} className="flex-1 rounded-xl border border-[#1e2130] px-4 py-2 text-sm text-[#5a5f72] hover:text-[#e8eaf0]">Cancel</button>
  ...
  return (
    <div className="min-h-screen bg-[#0a0c12] text-[#e8eaf0] p-4 sm:p-6 max-w-3xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-[#e8eaf0]">Study Groups</h1>
          <p className="text-sm text-[#4a4f62] mt-1">Team up, compete, and level up together</p>
        </div>
      ...
      {/* Join by invite code */}
      <div className="rounded-2xl border border-[#1e2130] bg-[#111318] p-4 mb-5">
        <p className="text-xs font-semibold uppercase tracking-widest text-[#4a4f62] mb-2">Join by Invite Code</p>
        <div className="flex gap-2">
          <div className="relative flex-1">
            <Hash size={13} className="absolute left-3 top-1/2 -translate-y-1/2 text-[#4a4f62]" />
            <input value={inviteCode} onChange={e => setInviteCode(e.target.value.toUpperCase())} placeholder="Enter 6-digit code…" maxLength={6} className="w-full rounded-xl border border-[#1e2130] bg-[#0a0c12] pl-8 pr-3 py-2 text-sm text-[#e8eaf0] placeholder-[#3a3d4a] outline-none focus:border-[#7C3AED] uppercase tracking-widest" />
          </div>
      ...
      <div className="flex gap-1 mb-4 bg-[#111318] rounded-xl border border-[#1e2130] p-1">
        <button onClick={() => setTab("discover")} className={`flex-1 rounded-lg py-2 text-xs font-medium transition-all ${tab === "discover" ? "bg-[#7C3AED] text-white" : "text-[#5a5f72] hover:text-[#e8eaf0]"}`}>
      ...
      {tab === "discover" && (
        <div>
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search groups…" className="w-full mb-4 rounded-xl border border-[#1e2130] bg-[#111318] px-3 py-2 text-sm text-[#e8eaf0] placeholder-[#3a3d4a] outline-none focus:border-[#7C3AED]" />
          {isLoading ? <div className="flex justify-center py-12"><div className="h-8 w-8 animate-spin rounded-full border-2 border-[#1e2130] border-t-[#7C3AED]" /></div> : (
            <div className="space-y-3">
              {filtered.length === 0 && <div className="text-center py-12 text-[#3a3d4a]"><Users size={40} className="mx-auto mb-3 opacity-30" /><p>No groups found</p></div>}
      ...
      {tab === "mine" && (
        <div className="space-y-3">
          {myGroups.length === 0 && <div className="text-center py-12 text-[#3a3d4a]"><Users size={40} className="mx-auto mb-3 opacity-30" /><p>You haven't joined any groups yet</p><button onClick={() => setTab("discover")} className="mt-3 text-xs text-[#7C3AED] hover:underline">Discover groups →</button></div>}
          {myGroups.map((g: any) => (
            <div key={g.id} className="rounded-2xl border border-[#7C3AED]/30 bg-[#111318] p-4">
              <div className="flex items-center gap-3 mb-3">
                <span className="text-3xl">{g.avatarEmoji}</span>
                <div className="flex-1"><p className="font-bold text-[#e8eaf0]">{g.name}</p><p className="text-xs text-[#4a4f62]">{g.memberCount} members · {g.groupXp?.toLocaleString() ?? 0} XP</p></div>
                {g.inviteCode && <div className="text-right"><p className="text-[9px] text-[#3a3d4a] uppercase tracking-wider mb-0.5">Invite</p><p className="text-sm font-bold text-[#7C3AED] font-mono tracking-widest">{g.inviteCode}</p></div>}
              </div>
              <div className="space-y-1 max-h-32 overflow-y-auto">
                {(g.members ?? []).slice(0, 5).map((m: any) => (
                  <div key={m.userId} className="flex items-center gap-2 text-xs">
                    ...
                    <span className="text-[#5a5f72] flex-1 truncate">{m.name}</span>
                    <span className="text-[#4a4f62]">{m.xpContribution?.toLocaleString() ?? 0} XP</span>
                  </div>
                ))}
              </div>
            </div>
