    ...
    <div className="min-h-screen bg-[#030308] text-white">
      ...
      <article className="mx-auto max-w-4xl px-6 py-16">
        ...
        <motion.div
          ...
          className="mb-12"
        >
          <div className="mb-4 inline-flex items-center gap-2 rounded-full border border-purple-500/30 bg-purple-500/10 px-3 py-1 text-[11px] font-bold uppercase tracking-widest text-purple-300">
        ...
        <motion.div
          ...
          viewport={{ once: true }}
          variants={fadeUp}
          className="mb-12 rounded-2xl border border-purple-500/20 bg-purple-500/5 p-6"
        >
          <p className="mb-2 text-xs font-bold uppercase tracking-wider text-purple-300">The Core Insight</p>
        ...
        >
          {[
            { value: "4×", label: "more learning per hour in deep focus vs. distracted study", color: "from-violet-500 to-purple-600" },
            { value: "23 min", label: "average time to regain focus after a single phone notification", color: "from-pink-500 to-rose-600" },
            { value: "↑ 40%", label: "better retention with spaced, focused sessions vs. marathon cramming", color: "from-teal-500 to-emerald-600" },
          ].map((stat, i) => (
            <div key={i} className="rounded-2xl border border-white/5 bg-white/[0.03] p-5 text-center">
              <div className={`mb-2 text-3xl font-black bg-gradient-to-r ${stat.color} bg-clip-text text-transparent`}>{stat.value}</div>
              <p className="text-xs leading-relaxed text-[#6B7280]">{stat.label}</p>
            </div>
          ))}
        </motion.div>
        ...
        <motion.section custom={4} initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp} className="mb-10">
          ...
          <p className="mb-4 text-[15px] leading-7 text-[#94A3B8]">
            Dr. Cal Newport, in his landmark book <em className="text-white">Deep Work</em>, defines the concept simply: 
          ...
          <ul className="mb-4 space-y-3">
            {[
              ...
              <li key={i} className="flex items-start gap-3 text-[14px] text-[#94A3B8]">
          ...
          {[
            ...
            >
              <div className="mb-4 flex items-center gap-3">
                <div className={`flex h-8 w-8 items-center justify-center rounded-xl bg-white/5 ${phase.accent}`}>
                  {phase.icon}
                </div>
                <div>
              ...
              <ul className="space-y-2.5">
                {phase.steps.map((step, j) => (
                  <li key={j} className="flex items-start gap-3 text-[13px] leading-relaxed text-[#94A3B8]">
        ...
        <motion.div
          ...
          viewport={{ once: true }}
          variants={fadeUp}
          className="rounded-2xl border border-purple-500/20 bg-gradient-to-br from-purple-500/10 to-pink-500/5 p-8 text-center"
        >
          ...
          <Link href="/signup">
            <motion.button
              whileHover={{ scale: 1.04, boxShadow: "0 0 30px 8px rgba(124,58,237,0.4)" }}
              whileTap={{ scale: 0.97 }}
              className="inline-flex items-center gap-2 rounded-2xl bg-gradient-to-r from-[#7c3aed] to-[#e879f9] px-8 py-3.5 text-[15px] font-bold text-white shadow-lg shadow-purple-900/30"
              // Analytics: blog_cta_click { article: "deep-study-guide", cta: "start-free" }
            >
              Start Your First Session Free <ArrowRight size={16} />
            </motion.button>
          </Link>
          <p className="mt-3 text-xs text-[#4B5563]">No credit card · Completely free · Takes 30 seconds</p>
        </motion.div>
          ...
          <div className="grid gap-3 sm:grid-cols-3">
            {[
              ...
              <Link key={i} href={article.href}>
                <div className="cursor-pointer rounded-xl border border-white/5 bg-white/[0.02] p-4 transition-colors hover:border-purple-500/20 hover:bg-purple-500/5">
