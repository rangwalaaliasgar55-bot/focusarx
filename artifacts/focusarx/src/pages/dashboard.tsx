...
import { PAGE, CARD, STAGGER, STAGGER_CHILD, SLIDE_UP, SLIDE_DOWN, POP, COUNT_UP } from "@/lib/animations";
...
function DailyHabitsWidget() {
  ...
  return (
    <div className="rounded-2xl border border-[var(--forge-border)] bg-[var(--card)] p-5 backdrop-blur-xl">
      ...
      {habits.length > 6 && (
        <Link to="/habits" className="mt-2 block text-center text-xs text-[#4B5563] hover:text-[#7C3AED]">
          ...
          return (
            <div key={m.key} className="space-y-1">
              <div className="flex items-center justify-between">
                <p className="text-xs text-[#CBD5E1] truncate flex-1 mr-2">
                  {m.icon} {m.title}
                </p>
...
function GoalsWidget() {
  ...
  if (isLoading) return (
    <div className="rounded-2xl border border-[var(--forge-border)] bg-[var(--card)] p-5 animate-pulse">
      ...
      <div className="space-y-2">{[...Array(3)].map((_, i) => (
        <div key={i} className="h-5 rounded bg-[rgba(255,255,255,0.04)]" />
      ))}</div>
    </div>
  );
    ...
    <div className="rounded-2xl border border-[rgba(255,255,255,0.06)] bg-gradient-to-r from-[rgba(124,58,237,0.08)] via-[rgba(79,70,229,0.05)] to-transparent p-5 mb-6">
      <div className="flex items-start justify-between gap-4 flex-wrap">
        ...
        <div className="flex gap-2 flex-wrap">
          <Link href="/" className="flex items-center gap-1.5 rounded-xl bg-gradient-to-r from-[#7C3AED] to-[#4F46E5] px-4 py-2 text-[12px] font-bold text-white hover:opacity-90 transition-opacity shadow-lg shadow-[0_0_12px_rgba(124,58,237,0.3)]">
            <Zap size={12} />
            Start Focus
          </Link>
                ...
                <div className="space-y-3">
                  ...
                  {stats.recentSessions.length === 0 ? (
                    <div className="rounded-2xl border border-dashed border-zinc-800/60 bg-zinc-950/30 px-6 py-10 text-center text-sm text-zinc-500">
                      Complete a focus session to see session insights here.
                    </div>
                  ) : (
