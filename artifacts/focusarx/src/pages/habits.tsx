...
function HeatmapCell({ date, done }: { date: string; done: boolean }) {
  return (
    <div
      title={date}
      className={`aspect-square rounded-sm ${done ? "bg-[#7C3AED]" : "bg-[#1e2130]"}`}
      style={{ minWidth: 10 }}
    />
  );
}
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4">
      <div className="w-full max-w-md rounded-2xl border border-[#1e2130] bg-[#111318] p-6">
        <div className="flex items-center justify-between mb-5">
          <h2 className="text-lg font-bold text-[#e8eaf0]">New Habit</h2>
          <button onClick={onClose}><X size={18} className="text-[#4a4f62]" /></button>
        </div>
        <div className="space-y-4">
          ...
          <div>
            <label className="text-xs font-semibold uppercase tracking-widest text-[#4a4f62] block mb-1.5">Icon</label>
            <div className="flex flex-wrap gap-1.5">
              {ICONS.map(ic => (
                <button key={ic} onClick={() => setForm(f => ({ ...f, icon: ic }))}
                  className={`text-xl rounded-lg p-1.5 transition-all ${form.icon === ic ? "bg-[#7C3AED]/30 ring-1 ring-[#7C3AED]" : "hover:bg-[#1e2130]"}`}>
                  {ic}
                </button>
              ))}
            </div>
          ...
          <div>
            <label className="text-xs font-semibold uppercase tracking-widest text-[#4a4f62] block mb-1.5">Color</label>
          ...
          <div>
            <label className="text-xs font-semibold uppercase tracking-widest text-[#4a4f62] block mb-1.5">Frequency</label>
            <div className="flex gap-2">
              {["daily", "weekdays", "weekends"].map(f => (
                <button key={f} onClick={() => setForm(fr => ({ ...fr, frequency: f }))}
                  className={`flex-1 rounded-lg py-1.5 text-xs capitalize transition-all ${form.frequency === f ? "bg-[#7C3AED] text-white" : "bg-[#0a0c12] text-[#5a5f72] hover:text-[#e8eaf0] border border-[#1e2130]"}`}>
                  {f}
                </button>
              ))}
            </div>
          ...
          <div className="flex gap-2 pt-2">
            <button onClick={onClose} className="flex-1 rounded-xl border border-[#1e2130] py-2 text-sm text-[#5a5f72] hover:text-[#e8eaf0]">Cancel</button>
            <button
    ...
    <div className="rounded-2xl border border-[#1e2130] bg-[#111318] p-4 transition-all hover:border-[#7C3AED]/20 shadow-3d">
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            <p className={`text-sm font-semibold truncate ${habit.completedToday ? "line-through text-[#4a4f62]" : "text-[#e8eaf0]"}`}>{habit.name}</p>
          ...
          <div className="flex items-center gap-3 mt-0.5">
            ...
            <span className="text-xs text-[#4a4f62]">{habit.totalCompletions} total</span>
            <span className="text-xs capitalize text-[#3a3d4a]">{habit.frequency}</span>
          </div>
        </div>

        <div className="flex items-center gap-1 shrink-0">
          <button onClick={() => setShowHeatmap(s => !s)} className="rounded-lg p-1.5 text-[#4a4f62] hover:text-[#e8eaf0] hover:bg-[#1e2130] transition-colors">
            {showHeatmap ? <ChevronUp size={14} /> : <BarChart2 size={14} />}
          </button>
          <button onClick={onDelete} className="rounded-lg p-1.5 text-[#4a4f62] hover:text-red-400 hover:bg-red-900/20 transition-colors">
            <Trash2 size={14} />
          </button>
        </div>
      </div>
      ...
      {showHeatmap && (
        <div className="mt-4 pt-4 border-t border-[#1e2130]">
          <p className="text-[10px] uppercase tracking-wider text-[#3a3d4a] mb-2">Last 90 days</p>
  ...
  return (
    <div className="min-h-screen bg-[#0a0c12] text-[#e8eaf0] p-4 sm:p-6 max-w-2xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-[#e8eaf0]">Habit Tracker</h1>
          <p className="text-sm text-[#4a4f62] mt-0.5">Build consistency, one day at a time</p>
        </div>
      ...
      {/* Daily progress ring */}
      {totalCount > 0 && (
        <div className="rounded-2xl border border-[#1e2130] bg-[#111318] p-5 mb-5">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-xs font-semibold uppercase tracking-widest text-[#4a4f62]">Today's Progress</p>
              <p className="mt-1 text-3xl font-black" style={{ color: completionPct === 100 ? "#4ade80" : "#7C3AED" }}>
                {completedCount}<span className="text-lg text-[#4a4f62] font-normal">/{totalCount}</span>
              </p>
              <p className="text-sm text-[#5a5f72]">{completionPct === 100 ? "🎉 All done for today!" : `${totalCount - completedCount} remaining`}</p>
            </div>
            <div className="relative">
              ...
              <div className="absolute inset-0 flex items-center justify-center">
                <span className="text-base font-bold text-[#e8eaf0]">{completionPct}%</span>
              </div>
            </div>
          </div>

          {stats && (
            <div className="grid grid-cols-3 gap-3 mt-4 pt-4 border-t border-[#1e2130]">
              <div className="text-center">
                <p className="text-lg font-bold text-amber-400">{stats.longestStreak}</p>
                <p className="text-[10px] text-[#4a4f62] uppercase tracking-wider">Best Streak</p>
              </div>
              <div className="text-center">
                <p className="text-lg font-bold text-[#a78bfa]">{stats.avgStreak}</p>
                <p className="text-[10px] text-[#4a4f62] uppercase tracking-wider">Avg Streak</p>
              </div>
              <div className="text-center">
                <p className="text-lg font-bold text-emerald-400">{stats.total}</p>
                <p className="text-[10px] text-[#4a4f62] uppercase tracking-wider">Active Habits</p>
              </div>
            </div>
          )}
        </div>
      ...
      {isLoading ? (
        <div className="space-y-3">
          {[1, 2, 3].map(i => <div key={i} className="h-20 animate-pulse rounded-2xl bg-[#111318]" />)}
        </div>
      ) : (habits as any[]).length === 0 ? (
        <div className="text-center py-16">
          <p className="text-5xl mb-4">🌱</p>
          <p className="text-lg font-semibold text-[#e8eaf0] mb-2">No habits yet</p>
          <p className="text-sm text-[#4a4f62] mb-6">Start building your first habit and track your consistency</p>
        ...
        <div className="space-y-3">
          ...
          {(habits as any[]).filter((h: any) => h.completedToday).length > 0 && (
            <div>
              <p className="text-xs font-semibold uppercase tracking-widest text-[#3a3d4a] mb-2 mt-4">Completed today ✓</p>
              {(habits as any[]).filter((h: any) => h.completedToday).map((h: any) => (
                <HabitCard
                  key={h.id} habit={h}
                  onComplete={() => completeHabit.mutate(h.id)}
