...
function TierCard({ tier, claimed, reached, premiumUnlocked, onClaim }: { tier: any; claimed: number[]; reached: boolean; premiumUnlocked: boolean; onClaim: (tier: number, track: "free" | "premium") => void }) {
  ...
  return (
    <div className={`relative rounded-2xl border p-3 transition-all ${isMilestone ? "border-amber-500/40 bg-amber-500/5" : "border-[#1e2130] bg-[#111318]"} ${!reached ? "opacity-40" : ""}`}>
      ...
      <div className="flex items-center justify-between mb-2">
        <span className="text-xs font-bold text-[#5a5f72]">Tier {tier.tier}</span>
        {reached ? <Check size={12} className="text-emerald-400" /> : <Lock size={12} className="text-[#3a3d4a]" />}
      </div>
      <div className="space-y-2">
        {/* Free track */}
        <div className={`flex items-center gap-2 rounded-lg p-2 ${reached && !freeClaimedThis ? "bg-emerald-500/10 border border-emerald-500/20" : "bg-[#0a0c12]"}`}>
          <Gift size={14} className="text-emerald-400 shrink-0" />
          <div className="flex-1 min-w-0">
            <p className="text-[10px] text-[#4a4f62]">FREE</p>
            <p className="text-xs font-semibold text-[#e8eaf0]">+{tier.freeReward.coins}🪙 +{tier.freeReward.xp}XP</p>
          </div>
        ...
        {/* Premium track */}
        <div className={`flex items-center gap-2 rounded-lg p-2 ${premiumUnlocked ? "bg-amber-500/10 border border-amber-500/20" : "bg-[#0a0c12] opacity-50"}`}>
          <Crown size={14} className="text-amber-400 shrink-0" />
          <div className="flex-1 min-w-0">
            <p className="text-[10px] text-[#4a4f62]">PREMIUM</p>
            <p className="text-xs font-semibold text-[#e8eaf0]">+{tier.premiumReward.coins}🪙 +{tier.premiumReward.xp}XP</p>
          </div>
          ...
          {!premiumUnlocked && <Lock size={11} className="text-[#4a4f62]" />}
        </div>
      </div>
    </div>
  );
    ...
    <div className="min-h-screen bg-[#0a0c12] text-[#e8eaf0] p-4 sm:p-6 max-w-3xl mx-auto">
      ...
      <div className="relative rounded-2xl border border-amber-500/30 bg-gradient-to-br from-amber-500/10 to-orange-500/5 p-6 mb-6 overflow-hidden">
        ...
        <div className="relative">
          ...
          <h1 className="text-3xl font-black text-[#e8eaf0]">Battle Pass</h1>
          <p className="text-sm text-[#5a5f72] mt-1">Earn XP, unlock rewards. Ends {data?.endsAt ?? "Sep 30"}</p>
          <div className="mt-4">
            <div className="flex justify-between text-xs mb-1.5">
              <span className="text-[#5a5f72]">Tier {tier} → {tier + 1}</span>
      ...
      {/* How to earn XP */}
      <div className="rounded-xl border border-[#1e2130] bg-[#111318] p-4 mb-6">
        <p className="text-xs font-semibold uppercase tracking-widest text-[#4a4f62] mb-2">Earn Battle Pass XP</p>
        <div className="grid grid-cols-3 gap-2 text-xs">
          {[["🎯", "Focus session", "+50 XP"], ["✅", "Task done", "+25 XP"], ["🔥", "Daily mission", "+100 XP"]].map(([icon, label, xp]) => (
            <div key={label} className="rounded-lg bg-[#0a0c12] p-2 text-center">
              <div className="text-xl mb-1">{icon}</div>
              <p className="text-[#5a5f72] text-[10px]">{label}</p>
      ...
      {/* Tiers grid */}
      <div>
        <p className="text-sm font-semibold text-[#e8eaf0] mb-3">All Tiers <span className="text-[#4a4f62] font-normal">(50 total)</span></p>
