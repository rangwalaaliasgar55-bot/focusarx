      ...
      <div className="space-y-2">
        ...
        <div className={`flex items-center gap-2 rounded-lg p-2 ${reached && !freeClaimedThis ? "bg-emerald-500/10 border border-emerald-500/20" : "bg-[rgba(255,255,255,0.02)]"}`}>
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs font-semibold text-[#E2E8F0]">+{tier.freeReward.value} {tier.freeReward.type}</p>
          </div>
        ...
        <div className={`flex items-center gap-2 rounded-lg p-2 ${premiumUnlocked ? "bg-amber-500/10 border border-amber-500/20" : "bg-[rgba(255,255,255,0.02)] opacity-50"}`}>
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs font-semibold text-[#E2E8F0]">+{tier.premiumReward.value} {tier.premiumReward.type}</p>
          </div>
          {premiumUnlocked && reached && !premiumClaimedThis && <button onClick={() => onClaim(tier.tier, "premium")} className="rounded-lg bg-amber-500 text-black px-2 py-1 text-[9px] font-bold hover:bg-amber-400">CLAIM</button>}
          {premiumClaimedThis && <Check size={12} className="text-emerald-400 shrink-0" />}
        ...
        <div className="relative">
          ...
          <div className="mt-4">
            <div className="flex justify-between text-xs mb-1.5">
              <span className="text-[#4B5563]">Tier {currentTier} → {currentTier + 1}</span>
          ...
          <div className="mt-3 flex items-center gap-3">
            <div className="flex items-center gap-1.5 rounded-full border border-amber-500/30 bg-amber-500/10 px-3 py-1"><Zap size={12} className="text-amber-400" /><span className="text-xs font-bold text-amber-400">Tier {currentTier} / 50</span></div>
            {!premiumUnlocked && (
