...
const SCHEDULE = [
  { time: "6:30 AM", label: "Wake + No screens (30 min)", desc: "Protect your morning cortisol peak — the brain's natural high-focus window", color: "text-amber-300" },
  { time: "7:00 AM", label: "Deep Block 1 (50 min)", desc: "Hardest subject or problem set. Peak cognitive window. Zero interruptions.", color: "text-violet-300" },
  { time: "7:50 AM", label: "Active Break (10 min)", desc: "Walk, stretch, hydrate. No phone. Let your brain consolidate.", color: "text-teal-300" },
  { time: "8:00 AM", label: "Deep Block 2 (50 min)", desc: "Second subject or continue first. Switch topics to benefit from interleaving.", color: "text-violet-300" },
  { time: "8:50 AM", label: "Session Debrief (10 min)", desc: "Log what you covered, rate your focus, identify what to revisit.", color: "text-emerald-300" },
  { time: "9:00 AM", label: "Rest of morning — lighter tasks", desc: "Review notes, watch lecture videos, admin tasks. Not deep work.", color: "text-[#6B7280]" },
];
...
export default function TwoHourStudyMethodPage() {
  ...
  return (
    <div className="min-h-screen bg-[#030308] text-white">
      ...
      {/* Nav */}
      <nav className="sticky top-0 z-50 border-b border-white/5 bg-[#030308]/90 backdrop-blur-2xl">
        <div className="mx-auto flex max-w-4xl items-center justify-between px-6 py-4">
          <Link href="/">
            <div className="flex items-center gap-2.5 cursor-pointer">
              <div className="flex h-7 w-7 items-center justify-center rounded-xl bg-gradient-to-br from-[#7c3aed] to-[#e879f9]">
                <svg viewBox="0 0 24 24" fill="white" className="h-3.5 w-3.5"><path d="M13 10V3L4 14h7v7l9-11h-7z" /></svg>
              </div>
          ...
          <Link href="/signup">
            <motion.button
              ...
              whileTap={{ scale: 0.97 }}
              className="rounded-xl bg-gradient-to-r from-[#7c3aed] to-[#e879f9] px-4 py-1.5 text-xs font-semibold text-white"
            >
              Try FocusArx Free →
            </motion.button>
          </Link>
      ...
      <article className="mx-auto max-w-4xl px-6 py-16">
        ...
        <motion.div initial="hidden" animate="visible" variants={fadeUp} className="mb-12">
          <div className="mb-4 inline-flex items-center gap-2 rounded-full border border-teal-500/30 bg-teal-500/10 px-3 py-1 text-[11px] font-bold uppercase tracking-widest text-teal-300">
          ...
          <h1 className="mb-4 text-3xl font-black leading-tight tracking-tight sm:text-4xl lg:text-5xl">
            The{" "}
            <span className="bg-gradient-to-r from-[#06D6A0] to-[#a78bfa] bg-clip-text text-transparent">
              2-Hour Study Method:
            </span>{" "}
          ...
          <div className="mt-6 flex flex-wrap items-center gap-4 text-xs text-[#4B5563]">
        ...
        <motion.section custom={1} initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp} className="mb-10">
          ...
          <p className="mb-4 text-[15px] leading-7 text-[#94A3B8]">
        ...
        <motion.div
          ...
          viewport={{ once: true }}
          variants={fadeUp}
          className="mb-10 rounded-2xl border border-teal-500/20 bg-teal-500/5 p-6"
        >
          <div className="flex items-start gap-3">
            <Lightbulb size={20} className="mt-0.5 shrink-0 text-teal-300" />
            <div>
              <p className="mb-2 text-sm font-bold text-teal-300">The Core Principle</p>
            ...
            {MYTHS.map((item, i) => (
              ...
              >
                <div className="mb-2 flex items-center gap-2">
                  <span className="rounded-full bg-red-500/10 px-2 py-0.5 text-[11px] font-bold text-red-400">MYTH</span>
                  <span className="text-sm font-bold text-white">{item.myth}</span>
                </div>
                <div className="flex items-start gap-2">
                  <span className="rounded-full bg-emerald-500/10 px-2 py-0.5 text-[11px] font-bold text-emerald-400 shrink-0">TRUTH</span>
                  <span className="text-[13px] leading-relaxed text-[#94A3B8]">{item.truth}</span>
                </div>
              </motion.div>
            ))}
          </div>
        ...
        <motion.section custom={4} initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp} className="mb-10">
          ...
          <p className="mb-6 text-[15px] leading-7 text-[#94A3B8]">
          ...
          <div className="mb-6 rounded-2xl border border-white/5 bg-white/[0.02] overflow-hidden">
            <div className="border-b border-white/5 p-4">
              <p className="text-sm font-bold text-white">Sample Morning Schedule (2-Hour Method)</p>
            </div>
          ...
          <div className="space-y-6">
            {[
              ...
              <div key={i} className="flex items-start gap-4">
                <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl border border-white/5 bg-white/[0.03] text-xl">
                  {item.icon}
                </div>
                <div>
                  <h3 className="mb-1.5 text-[15px] font-bold text-white">{item.title}</h3>
                  <p className="text-[13px] leading-relaxed text-[#94A3B8]">{item.text}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <motion.section custom={6} initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp} className="mb-10">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            {[
              { metric: "Focus Score", desc: "How many minutes of your 50-minute block were genuinely distraction-free? Target: 80%+ of session time", icon: <Target size={16} className="text-violet-400" /> },
              { metric: "Retention Rate", desc: "Can you recall the key concepts 24 hours later without reviewing? Test yourself daily.", icon: <BookOpen size={16} className="text-pink-400" /> },
              { metric: "Problem-Solving Speed", desc: "Are you solving the same type of problem faster week-over-week? Track average time per problem.", icon: <BarChart3 size={16} className="text-teal-400" /> },
              { metric: "Streak Consistency", desc: "Did you complete both deep blocks today? Yesterday? This week? Consistency outranks intensity.", icon: <TrendingUp size={16} className="text-amber-400" /> },
            ].map((item, i) => (
        ...
        <motion.div
          ...
          viewport={{ once: true }}
          variants={fadeUp}
          className="rounded-2xl border border-teal-500/20 bg-gradient-to-br from-teal-500/10 to-violet-500/5 p-8 text-center"
        >
          ...
          <Link href="/signup">
            <motion.button
              whileHover={{ scale: 1.04, boxShadow: "0 0 30px 8px rgba(6,214,160,0.25)" }}
              whileTap={{ scale: 0.97 }}
              className="inline-flex items-center gap-2 rounded-2xl bg-gradient-to-r from-[#06D6A0] to-[#7c3aed] px-8 py-3.5 text-[15px] font-bold text-white shadow-lg"
            >
        ...
        >
          <p className="mb-4 text-xs font-bold uppercase tracking-wider text-[#4B5563]">Related Articles</p>
          <div className="grid gap-3 sm:grid-cols-3">
            {[
              ...
              <Link key={i} href={article.href}>
                <div className="cursor-pointer rounded-xl border border-white/5 bg-white/[0.02] p-4 transition-colors hover:border-teal-500/20 hover:bg-teal-500/5">
                  <p className="mb-1 text-sm font-semibold text-white">{article.title}</p>
                  <p className="text-xs text-[#4B5563]">{article.desc}</p>
                </div>
              </Link>
            ))}
          </div>
