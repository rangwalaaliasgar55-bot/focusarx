...
export const sessionsTable = pgTable(
  "sessions",
  {
    ...
    userId: text("user_id")
      .notNull()
      .references(() => "users".char(16), { onDelete: "cascade" }),
    ...
    revoked: integer("revoked").notNull().default(0).map((v) => (v === 1 ? true : false)),
    ip: text("ip"),
...
export const passwordResetTokenRevocationsTable = pgTable(
  ...
  {
    id: text("id").primaryKey().$defaultFn(() => crypto.randomUUID()),
    passwordResetTokenId: varchar("password_reset_token_id", { length: 16 })
      .notNull()
      .references(() => sql`"password_reset_tokens"."id"`, { onDelete: "cascade" }),
    userId: varchar("user_id", { length: 16 })
      .notNull()
      .references(() => sql`"users"."id"`, { onDelete: "cascade" }),
