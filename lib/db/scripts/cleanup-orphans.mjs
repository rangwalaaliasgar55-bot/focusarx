...
BEGIN
  ...
  IF to_regclass('public.follows') IS NOT NULL THEN
    DELETE FROM public.follows a
      ...
      WHERE a.ctid > b.ctid
        AND a.follower_id = b.follower_id AND a.following_id = b.following_id;
  END IF;
END $$;
`;
...
BEGIN
  ...
  IF to_regclass('public.follows') IS NOT NULL AND NOT EXISTS (
    SELECT 1 FROM pg_indexes WHERE schemaname='public' AND indexname='follows_follower_following_unique') THEN
