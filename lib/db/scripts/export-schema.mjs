...
/**
 * The canonical schema as plain DDL, straight from `drizzle-kit export`.
 *
 * Reads the TypeScript schema only — no DATABASE_URL, no database access — so
 * it is safe to call from a build step. The output is the raw, non-idempotent
 * form (`CREATE TABLE "x"`), which is what `sync-schema.mjs` parses.
 */
export function exportRawSchemaSql() {
