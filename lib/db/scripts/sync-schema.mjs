 ...
 * statement runs in its own transaction: one failure (a UNIQUE that existing
 * data violates, say) is reported and the rest still apply.
 *
 * Usage
 * ─────
...
export function planSync(desired, live) {
  ...
  for (const constraint of desired.constraints) {
    if (creating.has(constraint.table)) {
      // The table is new, so nothing can conflict: add the FK verbatim.
...
function planConstraint(statements, manual, live, table, constraint) {
  ...
  statements.push({ kind: "add_constraint", table, name: constraint.name, sql: guardConstraint(table, constraint.name, ddl) });
}
...
export async function syncSchema({ connectionString, dryRun = false, strict = false, check = false, log = console } = {}) {
  ...
  try {
    ...
    await fillRowCounts(client, live, desired);
    ...
    for (const statement of statements) {
      ...
      try {
        ...
        failures.push({ label, code: err.code, message: err.message });
        log.error(`sync-schema: FAILED  ${label}: ${err.code ?? ""} ${err.message}`);
      }
    }
    ...
    if (!dryRun) log.log(`sync-schema: done — ${applied} applied, ${failures.length} failed, ${manual.length} left for review.`);
    else if (check && ok) log.log("sync-schema: check passed — the database already matches the schema.");
    return { ok, applied, failures, manual, planned: statements.length };
  } finally {
