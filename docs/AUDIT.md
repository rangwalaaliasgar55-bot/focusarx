...
**Still maintainer-only (unchanged):** nonce-based CSP (needs the SPA HTML served by the server rather than static hosting); CI activation (copy `docs/ci-workflow.example.yml` → `.github/workflows/ci.yml`; the sandbox GitHub App lacks the `workflows` permission); live-DB migrations (`docs/TIMESTAMPTZ_MIGRATION.md`, `docs/TABLE_CONSOLIDATION.md`, `lib/db/scripts/cleanup-orphans.mjs` refresh-token table).
