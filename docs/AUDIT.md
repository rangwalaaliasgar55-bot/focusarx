...
| `src/lib/env.ts` / `config.ts` | Zod-validated env, prod fail-fast, dev fallbacks (dev now drops only invalid keys — Phase 7). `config.ts` imports `node:crypto` explicitly. | — |
...
| D-4 | Legacy duplication: `posts` vs `social_posts`, `coin_transactions` vs `token_ledger`, `battlePasses/user_battle_pass_progress` vs `battle_pass_progress` vs `battle_pass_claims`, `user_pets` vs `pet_catalog/user_pet_inventory`. Multiple sources of truth → drift bugs. | P2 (needs consolidation plan, not a quick fix) — **plan delivered: `docs/TABLE_CONSOLIDATION.md`** (Phase 6 §22) |
| D-5 | `post_reactions` has **no unique (post_id,user_id,reaction)** — same user can spam duplicate reactions. `friendships` lacks unique (requester,addressee). `push_subscriptions.endpoint` not unique (dup rows per device). | Data hygiene | P1 — dedupe+unique patches shipped in `lib/db/scripts/cleanup-orphans.mjs` (Phase 7; maintainer executes) |
...
| D-7 | `password_reset_tokens` never purged (expires/used rows accumulate); `ai_call_log` grows unbounded; `notifications` no cleanup for deactivated users. | Table bloat | P2 — bounded purge patches shipped in `lib/db/scripts/cleanup-orphans.mjs` (Phase 7; maintainer executes) |
...
**Phase 4 — implemented (second pass)**
10. ✅ Explicit session state machine (`lib/sessionStateMachine.ts`): idle/active/paused/completed/cancelled/expired with an enforced transition table (14 unit tests); abandoned sessions finalize lazily on read (no cron): running-past-deadline auto-completes with server-owned duration + full rewards, paused/idle past a 2 h TTL archives as `expired` with no rewards; sync on an expired session returns 409 `SESSION_EXPIRED`; starting a new session finalizes a stale one instead of destroying it. Completion/streak/wallet logic deduplicated into shared transactional helpers.
11. ✅ Password change (`POST /api/auth/change-password`) — verifies current password, rejects no-op changes, revokes every refresh family, signs the caller out; account deletion (`DELETE /api/auth/account`) — password-confirmed (guests exempt), admins blocked, PII scrub of email logs before cascade, hard delete via FK cascades. UI: Account & security section on /profile with danger-zone confirmation dialog.
12. ✅ Distributed rate limiting: `UpstashRateLimitStore` (uses the existing `@upstash/redis` dep — zero new packages) wired into all limiters when `UPSTASH_REDIS_REST_URL`/`TOKEN` are set; memory store fallback otherwise; fixed-window INCR/PEXPIRE with per-limiter key namespaces (6 unit tests).
...
**Phase 6 — executed (fourth pass, this session)**
20. ✅ Client bearer retirement: `fetchSession` is cookie-first (`credentials:"include"`); a stored bearer is sent only if an old build left one in localStorage, and the client no longer stores new tokens on sign-in. The socket handshake uses the 60 s ticket (Phase 5), so localStorage no longer participates in auth at all for fresh sessions. `getToken()` remains only as a legacy-data fallback.
21. ✅ OpenAPI expansion: spec now covers 26 paths / 48 schemas (auth lifecycle ×14, sessions ×5, retention ×2 on top of the original break-free set), with `cookieAuth` + `bearerAuth` security and auth/sessions/retention tags. Orval clients regenerated (`lib/api-zod`, `lib/api-client-react`); new component schemas carry a `Dto` suffix to avoid operationId-derived Zod name collisions (TS2308).
22. ✅ Legacy table consolidation: verified repo-wide that `posts`, `post_likes`, `user_battle_pass_progress`, `study_buddies`, `shared_goals`, `leaderboard_snapshots` have **zero writers** (and the first two/five have zero readers); social routes already run on `social_posts`. Delivered the executable migration + drop plan in `docs/TABLE_CONSOLIDATION.md` (incl. a newly-found latent bug: `battlePassEnhanced.ts` gates tier claims on the never-written `user_battle_pass_progress` — code fix specified as Phase 0 of that doc). Actual `DROP TABLE` execution needs a live DB and is left to the maintainer.

**Phase 7 — closeout pass (fifth pass, this session)**
23. ✅ Battle-pass page repair (the last D-4 reader): `battlePassEnhanced.ts` read season XP from the never-written `user_battle_pass_progress`, so the client `/battle-pass` page rendered 0 progress for everyone and every tier claim failed. All three routes now read live `battle_pass_progress.season_xp`; tier math unified into `src/lib/battlePassTiers.ts` (pure, unit-tested) — this also fixed `/current` mapping a nonexistent `xpRequired` field and `/claim-all`'s hardcoded `tier * 500` gate. See `docs/TABLE_CONSOLIDATION.md` §2.
24. ✅ Standard 401 envelope: all 41 route-level `{ error: "Unauthorized" }` string responses now emit `{ error: { code: "UNAUTHORIZED", message } }` via `src/lib/httpErrors.ts` (`sendUnauthorized`). No client code keyed off the string shape (verified by grep before the sweep).
25. ✅ Small-bug closeout: removed the dead `verifyToken` branch (its fallback condition was self-contradictory); `pino-http` `genReqId` fallback now uses the same `req_` prefix as the app middleware; `env.ts` dev recovery drops **only** the offending keys and re-parses (one malformed var no longer discards every valid one); `adminAuth.ts` logs DB errors instead of silently failing closed.
26. ✅ DB-hygiene patches (D-5, D-7) added to `lib/db/scripts/cleanup-orphans.mjs` (maintainer-executed, idempotent): dedupe + unique indexes for `post_reactions(post_id,user_id,reaction)`, `friendships(requester_id,addressee_id)`, `push_subscriptions(endpoint)`; bounded purge of consumed/expired `password_reset_tokens` (>7d) and `ai_call_log` (>90d).
27. ✅ Client cleanup: removed the unused `completePersistedSession` duplicate of `syncFocusSessionToCloud` — `POST /api/sessions` now has a single client caller path (zero call sites referenced it; removal documented here per the no-silent-deletions rule).

**Still maintainer-only (unchanged):** nonce-based CSP (needs the SPA HTML served by the server rather than static hosting); live-DB maintenance (`docs/TIMESTAMPTZ_MIGRATION.md`, `docs/TABLE_CONSOLIDATION.md`, `lib/db/scripts/cleanup-orphans.mjs` refresh-token table) — full runbook in `docs/PRODUCTION_SETUP.md` §12. CI/CD is now activated: `.github/workflows/ci.yml` + `.github/workflows/deploy.yml` are committed (sources of truth in `docs/ci-workflows/`); if they do not appear in the Actions tab, commit them once with an account that has the `workflows` permission (`docs/PRODUCTION_SETUP.md` §6).
