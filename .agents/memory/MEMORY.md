...
- [404/500 sweep + route drift](error-404-500-sweep.md) — profile crashed because GET /api/gamification/badges omitted `stats`; /api/stats/streak, /api/social/requests(+accept/reject/delete), /api/mood, GET /api/sessions were all 404; Express 5 `req.body === undefined` normalised in app.ts (38 handlers destructure it); routeContract.test.ts now cross-checks every frontend call against registered routes
- ["All users are gone"](users-gone-schema-drift.md) — admin user list was empty because GET /admin/users bare-selected usersTable and 500'd on drift (users were never deleted); root cause: `drizzle-kit push` exits 0 when it aborts on a TTY prompt, so Vercel deploys silently drifted — push.mjs now fails the build; also admin live viewer (`live` block: online/focusingNow/current users) and chunkRecovery.ts so stale lazy chunks self-heal instead of needing a reload
